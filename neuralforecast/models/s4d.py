"""S4D (diagonal state-space) baseline, pure PyTorch. NeuralForecast ships no state-space model and
mamba-ssm needs a CUDA build, so S4D covers the SSM family here. The wrapper mirrors the CeNN
BaseModel plumbing so both run under the same protocol."""
...
class S4D(BaseModel):
    ...
    def configure_optimizers(self):
        """Two parameter groups: the SSM kernel parameters (log_dt, log_A_real, A_imag, C), which the
        official code tags with `_optim={'weight_decay': 0.0}`, get no weight decay; the rest get the
        configured weight decay. Decaying the kernel parameters shrinks the learned timescales and
        hurts long-range modeling. Otherwise the same optimizer, learning rate and scheduler as
