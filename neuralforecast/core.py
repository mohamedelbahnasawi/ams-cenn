...
from neuralforecast.models import (
    ...
    Autoformer,
    BiTCN,
    CeNN,  # FORK HAND-EDIT (not in nbs/core.ipynb): CeNN checkpoint registration — re-apply if nbdev_export is run
    DeepAR,
    DeepNPTS,
    DilatedRNN,
    DLinear,
...
MODEL_FILENAME_DICT = {
    ...
    "bitcn": BiTCN,
    "autobitcn": BiTCN,
    "cenn": CeNN,        # FORK HAND-EDIT (not in nbs/core.ipynb): re-apply if nbdev_export is run
    "autocenn": CeNN,
    "tide": TiDE,
    "autotide": TiDE,
    "deepnpts": DeepNPTS,
    ...
    def _no_refit_cross_validation(
        ...
        **data_kwargs,
        ...
        try:
            ...
            # FORK (low-mem CV index): ufp.cv_times materializes the id column as an OBJECT array
            # (each uid string repeated n_windows*h times) and then sorts the full frame. At
            # Traffic x H720 that is 1.73e9 rows: ~115 GB of strings + a billion-row string sort
            # + a string-keyed join -> kernel OOM even with 128 GB (measured on the H200 VM,
            # 2026-06-11; training itself peaked at 92 GB and passed). When every series spans
            # every window and uids are sorted (true for all our equal-length LTSF datasets), the
            # sorted order is a deterministic (series, window, step) enumeration, so we build the
            # frame DIRECTLY in final order with the id column as int32 Categorical codes
            # (~15x smaller, no sort, and the later join merges on codes). Anything else falls
            # back to the stock path, bit-identical to upstream.
            _sizes = np.diff(self.dataset.indptr)
            _n_windows_cv = (test_size - h) // step_size + 1
            _lowmem_ok = (
                os.environ.get("NF_CV_LOWMEM", "1") != "0"  # kill switch -> stock path
                and not isinstance(self.uids, pl_Series)
                and bool(np.all(_sizes >= test_size + 1))
                and pd.Index(self.uids).is_monotonic_increasing
            )
            if _lowmem_ok:
                _n_series = len(self.uids)
                _offsets = test_size - np.arange(_n_windows_cv) * step_size + 1   # [n_w]
                _cutoff_idx = (
                    self.dataset.indptr[1:][:, None] - _offsets[None, :]
                )                                                                  # [n_series, n_w]
                _step_idx = np.arange(1, h + 1)
                _ds_idx = (
                    _cutoff_idx[:, :, None] + _step_idx[None, None, :]
                ).reshape(-1)                                                      # uid-major flat
                _codes = np.repeat(
                    np.arange(_n_series, dtype=np.int32), _n_windows_cv * h
                )
                fcsts_df = pd.DataFrame(
                    {
                        id_col: pd.Categorical.from_codes(
                            _codes, categories=self.uids
                        ),
                        time_col: self.ds[_ds_idx],
                        "cutoff": np.repeat(
                            self.ds[_cutoff_idx.reshape(-1)], h
                        ),
                    }
                )
            else:
            ...
            # FORK (low-mem CV index): when the left frame's id column is categorical, cast the
            # right side to the SAME categories so pandas merges on int codes — merging
            # categorical-vs-object would coerce the billion-row column back to objects.
            _right = df[[id_col, time_col, target_col]]
            if isinstance(fcsts_df, pd.DataFrame) and isinstance(
                fcsts_df[id_col].dtype, pd.CategoricalDtype
            ):
                _right = _right.copy()
                _right[id_col] = pd.Categorical(
                    _right[id_col], categories=fcsts_df[id_col].cat.categories
                )
            return ufp.join(
                fcsts_df,
                _right,
