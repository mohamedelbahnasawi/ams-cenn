"""S4D (diagonal state-space) forecaster, the state-space baseline of this work.

S4DKernel, S4D and DropoutNd are the authors' implementation from
https://github.com/state-spaces/s4 (models/s4/s4d.py and the DropoutNd helper); Gu, Gupta, Goel, Re,
"On the Parameterization and Initialization of Diagonal State Space Models", NeurIPS 2022,
Apache-2.0. The only change is that `from src.models.nn import DropoutNd` is replaced by the copy
below. S4D is pure PyTorch and needs no custom CUDA kernels.

S4DModel1D (input embedding, residual S4D blocks with LayerNorm, linear forecast head) is the
wrapper that runs S4D under the same lookback, stride and scaler protocol as the other
...
# --- from state-spaces/s4 (models/s4/s4d.py), unchanged ---------------------------------------------
...
class S4DModel1D(nn.Module):
    """Forecaster around the S4D layer: input embedding, n_layers residual S4D blocks
    (S4D + residual + LayerNorm), linear forecast head. Maps [B, L, V] -> [B, H, V*c_out]."""
