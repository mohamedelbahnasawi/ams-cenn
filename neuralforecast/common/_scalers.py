...
def std_statistics(x, mask, dim=-1, eps=1e-6):
    ...
    # Protect against division by zero AND near-zero (flat-window) std.
    # Resetting only exactly-zero stds let near-flat windows (std ~1e-5)
    # divide the (normalized) horizon target by ~1e-5, inflating it ~1e6x
    # -> a loss blowup. Floor the std so flat windows can't explode.
    x_stds[x_stds == 0] = 1.0
    x_stds = x_stds.clamp_min(1e-2) + eps
