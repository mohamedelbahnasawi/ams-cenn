...
import torch
# ray is only needed for the Auto*/tune hyperparameter-search path, never for direct model
# training/inference. Make it optional so `import neuralforecast` works when ray is unavailable —
# e.g. when a code-integrity policy (Windows Smart App Control / WDAC) blocks ray's unsigned
# native DLL, which raises OSError (WinError 4551), not ImportError. (Fork change; nbs source not
# re-exported on this fork — keep this guard if regenerating.)
try:
    from ray import air, tune
    from ray.tune.integration.pytorch_lightning import TuneReportCallback
    from ray.tune.search.basic_variant import BasicVariantGenerator
    _RAY_AVAILABLE = True
except Exception:  # noqa: BLE001 — ImportError if absent, OSError if a policy blocks the DLL
...
class BaseAuto(pl.LightningModule):
    ...
    def __init__(
        ...
        valid_loss,
        config,
        search_alg=None,  # deferred: BasicVariantGenerator() default resolved in __init__ (import must work without ray)
        num_samples=10,
        time_budget=None,
        ...
        # Resolve the deferred search_alg default (kept off the signature so this module imports
        # without ray). Preserves prior behavior when ray is available; ray-backed Auto* search
        # is unusable without ray anyway, so leaving it None then is fine.
        if search_alg is None and _RAY_AVAILABLE:
