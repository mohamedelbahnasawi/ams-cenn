"""Robustness stress-test driver: test-time input-perturbation robustness.

For each (model, dataset, horizon, seed):
  1. load the trained checkpoint (read-only, no retraining);
  2. run a clean use_fitted cross_validation for the clean targets and the clean MSE;
  3. for every perturbation condition, run the model on a perturbed-input df and score its
     predictions against the clean targets (joined on window keys); report the MSE ratio.

Perturbation is applied to the df before any model scaler (global z-score domain), so it is the
same for all models. One atomic JSON per (cell, condition) under <out-root>/<cond>/results/,
skip-if-exists. Output paths are given explicitly; CENN_EXP_DIR is not used.
...
Usage:
  python -m experiments.run_robustness \
    ...
    --ckpt-root experiments/checkpoints --out-root experiments/robustness
"""
...
# Perturbation sweep (level 0 = clean, computed once per cell). The robustness criterion uses
# the mid-to-high levels (gauss>=0.5, spike>=0.03, mask>=48, scale>=0.25, shift>=1.0).
SWEEP = [
    ...
    ("dead", 1), ("dead", 2),   # dead-sensor family: 1 or 2 channels stuck at the mean
]
...
def run_cell(model, dataset, horizon, seed, ckpt_root: Path, out_root: Path, sweep=None):
    ...
    # Checkpoints are saved with save_dataset=False, so load() leaves these fit attributes
    # unset; use_fitted's pre-eval snapshot needs them to exist. The eval rebuilds them from
    # the passed df, and nf is discarded afterwards, so None is safe.
    for _attr in ("dataset", "uids", "last_dates", "ds"):
        if not hasattr(nf, _attr):
            setattr(nf, _attr, None)
    alias = nf.models[0].alias

    # clean pass: targets and denominator
...
def main():
    ...
    ap.add_argument("--out-root", default="experiments/robustness")
