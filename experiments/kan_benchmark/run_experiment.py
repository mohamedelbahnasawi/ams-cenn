...
from neuralforecast.models import KAN, MLP, NBEATS, NHITS, CeNN
results = []
...
if __name__ == "__main__":
    ...
    kan_model = KAN(input_size=2*horizon, h=horizon, scaler_type='robust', early_stop_patience_steps=3, accelerator="gpu", logger=True)
    mlp_model = MLP(input_size=2*horizon, h=horizon, scaler_type='robust', max_steps=1000, early_stop_patience_steps=3, accelerator="gpu", logger=True)
    ...
    cenn_model = CeNN(input_size=2*horizon, h=horizon, scaler_type='robust', max_steps=1000, early_stop_patience_steps=3, n_series=Y_df['unique_id'].nunique())

    MODELS = [kan_model, mlp_model, nbeats_model, nhits_model, cenn_model]
    MODEL_NAMES = ['KAN', 'MLP', 'NBEATS', 'NHITS', 'CeNN']
