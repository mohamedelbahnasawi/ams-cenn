...
Re-runnable; reads the current state of results/.
...
def load_all_results() -> pd.DataFrame:
    ...
    A published (literature) cell is loaded only if there is no run for that
    (model, dataset, horizon). Once a baseline has been re-run, its transcription in published/
    stays on disk as the appendix reproduction reference but is not merged here, otherwise the
    seed mean would blend a published value (seed=0) with the re-run seeds.
    """
    records = []

    # Count seeds per cell so a published number hidden by a 1-seed run can be flagged
    # (precedence keys on (model, dataset, horizon), not on seed completeness).
    run_seeds = {}
    ...
    # Published baselines, only for cells without a run.
    ...
    for f in sorted(PUBLISHED_DIR.glob("*.json")):
        try:
            ...
            if key in run_seeds:
                skipped_overridden += 1     # run exists; literature value stays appendix-only
    ...
    # Keep only the LTSF benchmark datasets so stray result JSONs from other runs cannot enter
    # the tables.
