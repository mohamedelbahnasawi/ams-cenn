"""Synthetic-chaos probe: does the cellular recurrence capture nonlinear dynamics when run longer?

On the LTSF benchmarks the residual of the anchored linear map is close to unpredictable, and the
shipped trunk is near-linear (K=2 steps of a contractive map, input held as a constant bias). This
script measures, on synthetic nonlinear data, whether (a) more iterations and (b) relaxing the
contraction certificate buy anything over a linear map and over a generic MLP.
