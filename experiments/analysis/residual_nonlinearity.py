"""Tests for remaining nonlinear structure in the residuals of the anchored linear fit.

Under the anchored protocol the skip-only model (== NLinear) matches or beats the full AMS-CeNN on
ETT+Weather. If its residual is white, no nonlinear pathway can add point accuracy on these
benchmarks. Two tests on the skip-only residuals of the test windows:

  1. BDS test (Brock-Dechert-Scheinkman) on the one-step-ahead residual series per channel:
     rejects iid at the 5% level if any dependence (linear or nonlinear) remains.
  2. Predictability probe: from a window of past residuals, predict the residual at lead h with
     ...
     chronological split. Reading: R^2 ~ 0 for both, nothing left to model; linear ~ nonlinear > 0,
     more linear capacity would help; nonlinear > linear, a nonlinear pathway could help.

Data: the SkipOnly-Anc (and AMS-Anc) checkpoints, clean stride-1 test windows as evaluated by the
robustness harness. Residual r_t = y_t - yhat_t.
