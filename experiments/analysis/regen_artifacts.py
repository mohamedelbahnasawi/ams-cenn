...
already-trained variant without retraining: load the checkpoint, run the clean stride-1 test
evaluation (the same pass the robustness harness uses), then call the runner's artifact savers.

The artifacts are large and not saved by default; recomputing them from the checkpoint gives the
same values as the original run because training is deterministic.
