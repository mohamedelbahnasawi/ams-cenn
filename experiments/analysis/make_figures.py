#!/usr/bin/env python
"""Build the results-driven paper figures.

Joins the seed-mean MSE (experiments/results/, via aggregate.load_all_results) to the efficiency
profile (experiments/efficiency/*.json, from profile_efficiency.py) and passes the rows to the
plotting functions. main() writes the lookback sweep, the per-dataset horizon grid, the retention
profile, the rank heatmap, the worst-case bar chart, the rank box plots, the forecast panels and
the contraction audit; the Pareto, efficiency-panel and K-sweep figures are no longer written.
--dataset/--horizon select the anchor cell for the artifact-based figures (default ETTh1, H96).
...
# Display names for CeNN variants, keyed by the bare variant (CeNN_ stripped); missing keys fall
...
def build_rows(dataset, horizon, mse_scope, eff):
    ...
    the scoped mean accuracy, not a single anchor cell."""
    rows = []
    for (model, ds, h), e in eff.items():
        ...
        rows.append({
            ...
            # Only the headline CeNN variant is labelled; other CeNN variants render as faded,
            # unlabelled stars. Baselines keep their labels.
...
def build_ksweep(dataset, horizon, mse, eff, std=None):
    """K × integrator points. C1-BoundedTau supplies the K8-Euler point (keyed on cenn_K/integrator,
    ...
    for v, (integ, k) in kvars.items():
        ...
        if e is None or m is None:
            ...
            continue
        # check the variant's declared K/integrator against kvars
...
def tau_figure(dataset, horizon):
    ...
    (model.py: self.last_tau = 1 - alpha), shape (latent_channel, time). The plotted quantity is the
    retention alpha = 1 - tau (about 0.90). alpha is nearly constant over the lookback (time std
    about 1e-4) but varies by latent channel (channel std about 1e-2), so the figure shows the
    time-averaged per-channel value."""
    import numpy as np
...
def ablation_bars(df, datasets=None):
    """fig_ablation_bars: leave-one-out Δ MSE of each ablation variant against the headline, computed
    on matched cells only: for each variant, restrict to the (dataset, horizon, seed) cells present
    in both that variant and the headline, then take the mean difference (variants cover 72-124
    different cells, so unmatched means are not comparable). Positive Δ means removing the component
    hurts. `datasets` optionally restricts the cells to a dataset regime."""
    ...
    # The no-skip variant (headline minus the zero-init linear skip) lives in CENN_VARIANTS_MAIN,
    # not _ABLATION, so it is added explicitly.
    SKIP_ABL = ROLES["no_skip"]
    # Two groups: REMOVED = a component of AMS-CeNN taken out (Δ is its contribution);
    # ALT = a design choice AMS-CeNN does not use swapped in (Δ is how much worse it is).
    # The K/integrator sweep is excluded (own figure, fig_k_integrator).
    ...
    for v in arch + [SKIP_ABL]:
        ...
        if len(shared) < 8:                      # too few matched cells to compare
...
def mse_boxplots(df, models, datasets=None):
    ...
    the headline regime `datasets` (ETT+Weather); ECL/Traffic are excluded here but appear in the
    main table."""
    d = df if datasets is None else df[df["dataset"].isin(datasets)]
    dist = {}
    for m in models:
...
def robustness_fig(mse, models, datasets=None):
    """fig_robustness: each model's worst-dataset relative error, i.e. % above the best model per
    cell, mean over horizons, then the max over datasets."""
    import statistics
    ds = list(DATASETS_ALL if datasets is None else datasets)
    items = []
    for m in models:
...
def lsweep_fig(datasets=("ETTh1", "ETTh2", "Weather"), horizon=96):
    """fig_lsweep: lookback sensitivity, full model vs the variant without the skip, mean MSE over
    `datasets` at one horizon, L in {96,192,336,512}."""
    import statistics
    Ls = [96, 192, 336, 512]

    def m(variant, L):
        root = str(RESULTS_DIR) if L == 512 else str(EXPERIMENTS_DIR / "controls" / "lookback" / f"L{L}" / "results")
...
def horizon_grid(mse, datasets, models):
    """Per-dataset small multiples of MSE vs horizon (own y-scale per panel). `models` = AMS-CeNN plus
    a few of the best and worst baselines; the plotter highlights AMS-CeNN and clips an off-scale
    outlier so the rest stays readable."""
    panels = {}
    for d in datasets:
...
def rank_boxplots(mse, models, datasets=None):
    ...
    complete (dataset, horizon) blocks. Scale-free, unlike raw MSE pooled across datasets; the
    same ranks enter the Friedman/CD analysis."""
...
def forecast_grid(datasets, horizon, model=None):
    ...
    from the --save-artifacts prediction .npz. Picks the median-error full-horizon window per dataset."""
    import numpy as np
    import pandas as pd
    ...
    for ds in datasets:
        ...
        # representative window: the one whose error is closest to the median
    ...
    if not panels:
        print("[forecast-grid skipped: no prediction artifacts (run with --save-artifacts)]")
        return None
    return plotting.fig_forecast_grid(panels, FIG_DIR)


def forecast_panels(specs, model=None):
    """fig_forecast_grid for an explicit list of (dataset, horizon) pairs, one median-error window per
    panel. Used for the qualitative figure (2 datasets x 2 horizons)."""
    import numpy as np
    import pandas as pd
    model = model or f"CeNN_{CENN_MAIN_VARIANT}"
    pred_dir = ARTIFACTS_DIR / "predictions"
    ...
    for ds, H in specs:
        ...
        # Representative window = median-error window among the non-degenerate windows (truth std at
        # or above the dataset median, not quantised): ETTm2 has near-constant series whose
        # median-error window is a flat line. The selection rule is stated in the figure caption.
...
def contraction_figure(dataset, horizon):
    """fig_contraction (C1 stability): learned vs capped feedback operator norm ||A_eff|| per
    branch-channel pair, from an `aeff` artifact of a capped CeNN run, plus the per-step
    contraction factor. Post hoc, no retraining."""
    import numpy as np
    ...
    for v in [CENN_MAIN_VARIANT, "C1-BoundedTau"]:
        ...
        if not hits:
            continue
        # use the seed with the largest learned norm, where the cap is closest to binding
        ...
        # ||J|| <= alpha + (1 - alpha) * ||A_eff||; contractive iff alpha_max < 1 and ||A_eff|| < 1.
        # With alpha_max ~ 0.99 the margin 1 - factor is set by the retention rate, not by ||A_eff||.
        ...
        if n_binding == 0:
            # cap never binds: learned and effective norms coincide, so only one norm curve is drawn
...
def scale_uq_figures(dataset, horizon, model=None):
    ...
    # window, overlapping windows), so rho is reported with a window-level block-bootstrap 95% CI
    # (whole windows resampled).
...
def main():
    ...
    if not eff:
        # only the efficiency figures need the eff JSONs; warn and continue with the rest
    ...
    made = []
    # The Pareto, efficiency-panel and K-sweep figures are not part of the paper (compute is
    # reported in the efficiency table; the K surface is flat, dMSE < 0.001 over K in {2,4,8}).
    # The L-sweep is the sensitivity figure.
    lsf = lsweep_fig()
    if lsf:
        made.append(lsf)

    hl_models = [f"CeNN_{CENN_MAIN_VARIANT}"] + BASELINES_ALL
    # per-dataset horizon small multiples: AMS-CeNN + 3 best + 2 worst baselines
    ...
    df = load_all_results()
    if not df.empty:
        # The leave-one-out ablation bar chart is not generated: every ABL-* variant lacks the linear
        # skip, so each Δ is dominated by the absent skip. The build-up table (tab:ablation) is used.
        hm = rank_heatmap(mse, hl_models)                # §1 no-champion motivation (per-dataset ranks)
        if hm:
            made.append(hm)
        rb = robustness_fig(mse, hl_models)              # worst-case relative error
        if rb:
            made.append(rb)
        bx = rank_boxplots(mse, hl_models)               # scale-free per-block ranks (appendix/supplementary)
        if bx:
            made.append(bx)
    # Qualitative figure: 2 datasets x 2 horizons, median-error windows.
    ...
    if ct:
        made.append(ct)
    # The C2 UQ figures (scale disagreement, spread vs error) are not part of the paper.
