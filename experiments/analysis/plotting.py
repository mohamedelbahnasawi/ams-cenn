#!/usr/bin/env python
"""Paper figures, built on the SciencePlots `ieee` style.

Shared style (Times, IEEE column widths, colorblind-safe palette, label repulsion with leader
lines, vector PDF + 600-dpi PNG) and one function per figure. The callers in this package supply
the data; running the module directly renders the whole suite with illustrative data.
"""
...
def apply_style():
    ...
    plt.rcParams.update({
        # SciencePlots ieee uses ~8pt; bumped for legibility.
...
# ---------------------------------------------------------------------------
# 3. K x integrator sweep (accuracy vs MACs)
# ---------------------------------------------------------------------------
...
def fig_cd_diagram(mean_ranks, cd, outdir, name="fig_cd_diagram", note=None):
    ...
    fig, ax = plt.subplots(figsize=(COL_W, STEP * n + 0.55))  # single-column width, displayed ~1:1
    ax.set_xlim(lo - 0.6, hi + 0.6)
    ax.set_ylim(lowest - 0.28, y0 + 0.6)            # tight top/bottom margins
    ax.axis("off")
    ax.plot([lo, hi], [y0, y0], "k-", lw=1.5)               # rank axis
    ...
    # CD clique bars: maximal cliques only (consecutive runs within cd); a clique is kept only if
    # its right endpoint extends past every previously kept clique, so nested bars are not drawn.
    cliques, last_j = [], -1
    i = 0
    while i < n:
        j = i
...
# ---------------------------------------------------------------------------
# 5. Per-channel retention profile (interpretability, C1)
# ---------------------------------------------------------------------------
def fig_tau_profile(mean_alpha, std_alpha, outdir, name="fig_tau_heatmap", model_label="AMS-CeNN",
                    note=None):
    """Learned retention alpha = 1 - tau per latent channel (sorted). alpha is nearly constant over
    the lookback (time std ~1e-4), so the figure shows the time-averaged per-channel value with a
    +-std band over input windows rather than a time x channel heatmap. The channel spread is
    printed on the axes because the auto-scaled y-range is small."""
    apply_style()
...
# ---------------------------------------------------------------------------
# 5b. Retention heatmap (time x channel)
# ---------------------------------------------------------------------------
...
def fig_ablation_bars(items, outdir, name="fig_ablation_bars"):
    """items: (label, Δ, highlight, group). Δ = variant_MSE − headline_MSE on matched cells. Two
    colour-coded, gap-separated groups: 'Removed component' (Δ = the accuracy that component
    contributes) and 'Alternative design' (Δ = how much worse the swap is). The highlighted item
    (the linear skip) is drawn in the headline accent."""
...
def fig_scale_disagreement(spread, outdir, name="fig_scale_disagreement", scales=None):
    ...
    # branches are diagnostic probes of the multi-scale latent (forward_branches heads each branch
    # separately), not independent forecasts
...
# ---------------------------------------------------------------------------
# 10. Contraction audit (C1 stability)
# ---------------------------------------------------------------------------
...
def fig_forecast_grid(panels, outdir, name="fig_forecast_grid", ncols=None, model_label="AMS-CeNN"):
    ...
    for idx, p in enumerate(panels):
        ...
        # seed-ensemble band: a lighter tint than the prediction line, with a thin edge, so the
        # narrow band stays visible
...
def fig_horizon_curves(series, horizons, outdir, name="fig_horizon_curves", scope_note=None):
    """series: dict model -> {'mse': [per-horizon], 'family', 'headline'}. The headline is a bold
    orange star line; every other method gets a distinct colour from a qualitative map (the family
    palette would give the four MLPs one colour) and a thinner line."""
    apply_style()
...
# ---------------------------------------------------------------------------
# 13. Sensitivity lines, worst-case bars, horizon grid, rank heatmap, radar, rank/MSE box plots
# ---------------------------------------------------------------------------
...
def fig_robustness(items, outdir, name="fig_robustness", highlight="AMS-CeNN", cap=35.0):
    """items: list of (label, pct) where pct = % above the best model on the worst dataset. Sorted
    ascending; the highlighted model is accented. Bars beyond `cap` are clipped and annotated with
    their value."""
    apply_style()
...
def fig_horizon_grid(panels, horizons, outdir, name="fig_horizon_curves", highlight="AMS-CeNN",
    ...
    y-scale), shared legend in the spare cell, the highlighted model accented."""
    import math
    apply_style()
    ...
    for ai, d in enumerate(dsets):
        ...
        for m in models:
            ...
            handles[m] = ln
        # per-panel y-cap: if one model is a big outlier (>1.8x the next), clip it and annotate which
        # model goes off-scale
...
def fig_rank_heatmap(models, datasets, rankmat, outdir, name="fig_rank_heatmap",
    ...
    """rankmat[i][j] = integer rank (1=best of k) of models[i] on datasets[j]. Green to red, rank printed
    in every cell; the highlighted model's row is boxed."""
    apply_style()
...
def fig_radar(categories, series, outdir, name, title, highlight="AMS-CeNN", note=None):
    ...
    (higher = better), one per category. The highlighted series is drawn thicker and filled."""
    N = len(categories)
...
def fig_rank_boxplots(dist, outdir, name="fig_rank_boxplots", n_blocks=28, k=12):
    ...
    headline accent; y-axis inverted so rank 1 is at the top."""
    apply_style()
...
def fig_mse_boxplots(dist, outdir, name="fig_mse_boxplots", scope_note=None):
    ...
    ax.set_xticklabels(labels, rotation=40, ha="right", fontsize=7.5)
    ax.set_ylabel(r"MSE ($\downarrow$)")
    # Cap the y-axis at the 98th percentile of the pooled values and annotate how many points sit above.
...
# ---------------------------------------------------------------------------
# 14. Spread-vs-error scatter (scale disagreement vs absolute error)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Demo: render the whole suite with illustrative data
# ---------------------------------------------------------------------------
def _demo(outdir):
