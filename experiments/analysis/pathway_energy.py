...
The forecast is y = trunk(x) + skip(x) (model.py: `out = self._head(...)`, then
`out = out + self.skip(...)`). This script captures both terms on the test windows of a trained
checkpoint, no retraining, and reports how ||y||^2 divides between them:

    ||y||^2 = ||skip||^2 + ||trunk||^2 + 2<skip, trunk>

The pathways are not orthogonal, so the cross-term is reported alongside the two squared norms; it
can be large and negative (the trunk partly cancelling the skip) or positive. All three terms are
printed as shares of ||y||^2 and sum to 100%.

Energy is not accuracy: a pathway can carry a large share of the output norm while contributing
little to predictive skill. The frozen-pathway experiment measures the accuracy contribution.
...
def main() -> None:
    ...
    if rows:
        ...
        print("\nShares sum to 100%. A negative cross-term means the pathways partly cancel; "
              "a positive one\nmeans they reinforce.")
