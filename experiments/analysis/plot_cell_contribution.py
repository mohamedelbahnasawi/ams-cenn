"""Window-level contribution of the cellular path on every dataset.

By eq. (9) the forecast splits exactly into the linear path y_lin (the residual alone, passed through the
inverse LVN) and the cellular correction y_cell = y - y_lin.  For every test window the script computes
the MSE of the full forecast and of the linear path alone against the truth (all channels of the window
pooled) and reports the relative change  (MSE_full - MSE_lin) / MSE_lin  in percent (negative = the
cellular path helps), plus the per-dataset share of the energy of (y - x_L) carried by the cellular path.

One trained checkpoint per dataset (seed 1, H=96 by default), no retraining. Only [W, H, V] outputs are
kept in memory, so the high-cardinality datasets fit.
...
def run_cell(dataset: str, H: int, seed: int):
    ...
    W, Hh, V = y.shape
    L = x0.shape[1]
    # ground truth by index arithmetic, checked against the captured input of channel 0; streamed per
