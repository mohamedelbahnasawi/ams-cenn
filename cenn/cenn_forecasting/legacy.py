...
class CeNNCell1D(nn.Module):
    ...
    def precompute(self, u: torch.Tensor):
        """Compute the terms that do not depend on the state v, once before the K-loop.
...
class CeNNLayer1D(nn.Module):
    ...
    def forward(self, u: torch.Tensor):  # u [B, C, L]
        # tau, masked weights and control B*u + I do not depend on v; compute once.
...
class CeNNModel1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        adaptive_tau: bool = False,           # C1
        dilation_schedule: str = "none",      # C2
        var_mix: bool = False,                # cross-variable mixing (VarMix)
        pointwise_mix: bool = False,          # latent channel mixing
